...
.change-icon {
  ...
  font-size: 1rem; /* no token: 1rem is between --text-base (0.9375rem) and --text-lg (1.125rem) */
}

.chart-card {
  margin-bottom: 1.75rem; /* no token: 1.75rem not on spacing scale */
}
...
.legend-dot {
  width: 12px;
  height: 12px;
  border-radius: 3px; /* no token: 3px is below --radius-sm (6px) */
}

.legend-dot.procurement { background: #3b82f6; } /* no token: chart category color */
.legend-dot.operational { background: #8b5cf6; } /* no token: chart category color */
.legend-dot.labor { background: #10b981; } /* no token: chart category color */
.legend-dot.overhead { background: #f59e0b; } /* no token: chart category color */
.legend-dot.revenue-color { background: var(--slate-900); }
.legend-dot.cost-color { background: #ef4444; } /* no token: #ef4444 is not --color-danger (#dc2626) */
...
.cost-card {
  border-left: 4px solid #ef4444; /* no token: #ef4444 is not --color-danger (#dc2626) */
}

.profit-card {
  border-left: 4px solid #3b82f6; /* no token: raw blue, not --color-accent (#2563eb) */
}

.stat-meta {
  margin-top: var(--space-2);
  font-size: 0.813rem; /* no token: between --text-xs (0.75rem) and --text-sm (0.875rem) */
  color: var(--color-text-secondary);
}
...
.revenue-bars {
  ...
  max-width: 80px;
  display: flex;
  gap: 6px; /* no token: 6px is not on the 4px spacing scale */
...
.revenue-bar, .cost-bar {
  width: 50%;
  max-width: 30px;
  border-radius: 6px 6px 0 0; /* compound shorthand: --radius-sm applies to full-corner only */
  transition: all 0.3s ease; /* intentional: 0.3s differs from --transition-base (150ms) */
  cursor: pointer;
  min-height: 4px;
}
...
.cost-bar {
  background: #ef4444; /* no token: #ef4444 is not --color-danger (#dc2626) */
}
...
.bar-chart {
  display: flex;
  gap: var(--space-6);
  height: 350px; /* no token: fixed pixel chart height */
}
...
.stacked-bar {
  ...
  padding-bottom: var(--space-8);
  cursor: pointer;
  transition: opacity 0.2s ease; /* intentional: 0.2s differs from --transition-base (150ms) */
}
...
.bar-segment:first-child {
  border-radius: 0 0 6px 6px; /* compound shorthand: --radius-sm applies to full-corner only */
}
...
.bar-segment.procurement { background: #3b82f6; } /* no token: chart category color */
.bar-segment.operational { background: #8b5cf6; } /* no token: chart category color */
.bar-segment.labor { background: #10b981; } /* no token: chart category color */
.bar-segment.overhead { background: #f59e0b; } /* no token: chart category color */
...
.two-column-grid {
  ...
  gap: 1.75rem; /* no token: 1.75rem not on spacing scale */
}
...
.category-bar-container {
  width: 100%;
  height: 8px; /* no token: fixed pixel track height */
  background: var(--slate-100);
  border-radius: 4px; /* no token: 4px is between grid steps, below --radius-sm (6px) */
  overflow: hidden;
}

.category-bar {
  height: 100%;
  background: linear-gradient(90deg, #3b82f6 0%, #2563eb 100%); /* no token: gradient stops can't be tokenized inline */
  border-radius: 4px; /* no token: 4px is between grid steps, below --radius-sm (6px) */
  transition: width 0.6s ease; /* intentional: 0.6s animation, not a base transition */
}
...
.transactions-table-container {
  overflow-y: auto;
  max-height: 400px; /* no token: fixed pixel scroll cap */
}
...
.transactions-table th {
  text-align: left;
  padding: 0.625rem var(--space-3); /* 0.625rem has no token: between --space-2 (0.5rem) and --space-3 (0.75rem) */
  font-weight: var(--font-semibold);
  color: var(--slate-600);
  font-size: var(--text-xs);
...
.transactions-table tbody tr {
  cursor: pointer;
  transition: background-color 0.15s ease; /* intentional: 0.15s differs from --transition-base (150ms ease-out) */
}
